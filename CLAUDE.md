...
- 과거 달의 결산은 그대로 보존해야 한다. 그래서 규칙 변경은 조건문이 아니라 「이 달부터」 상수로 넣는다 - `GHOST_FROM`(고스트 전환)·`JOIN_FROM`(합류)·`LEFT_FROM`(탈퇴)·`NO_DUES_FROM`(회비 폐지)·`CARD_LEFT_FIRST_FROM`(법인카드 카드 표기)이 그 패턴이다. 명단에 사람을 더할 때도 `ACTIVE`/`GHOST` 배열을 직접 늘리지 말고 `JOIN_FROM` 에 합류 달을 적는다.
...
- `CARD_LEFT_FIRST_FROM='2026-09'` - 2026년 9월부터 법인카드 카드는 사용액이 아니라 남은 한도를 큰 숫자로 보여준다. 한도는 `allOf(mk).length × SUPPORT_PER`(명단 인원수 × 3만원)라 명단이 늘면 같이 오른다.
